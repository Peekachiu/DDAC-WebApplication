...
export default function ResidentSportFacilityBooking({ user }) {
  ...
  const getFacilityIcon = (name) => {
    ...
    if (lowerName.includes('pool') || lowerName.includes('swim')) return <Users className="h-6 w-6 text-[#f37021]" />;
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {facilities.length === 0 ? (
              ...
              facilities.map((facility) => (
                <div
                  key={facility.id}
                  className="flex items-center gap-4 rounded-lg shadow-md bg-white/40 p-4 hover:bg-white/60 transition-colors"
                >
